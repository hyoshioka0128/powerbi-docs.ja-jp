...
- PowerShell スクリプトのユーザーは [Connect-PowerBIServiceAccount](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount) コマンドレットを使用してサインインし、メッセージが表示されたら Power BI 管理者の資格情報を入力する必要があります。 アクティビティ ログ API を使用するには、管理者の権限が必要です。

> [!IMPORTANT]
> 管理者の権限がないユーザーは、サンプル スクリプトでコード フラグメントを実行できません。
