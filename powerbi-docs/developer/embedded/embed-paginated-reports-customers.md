...
埋め込みトークンを生成します。これは Power BI 埋め込み分析のクライアント API から使用できます。 Power BI のページ分割されたレポートを埋め込むための埋め込みトークンを作成するには、[Reports GenerateTokenInGroup](/rest/api/power-bi/embedtoken/reports_generatetokeningroup) API を使用します。
...
### <a name="load-an-item-using-the-client-apis"></a>クライアント API を使用してアイテムを読み込む

Power BI 埋め込み分析のクライアント API を使用して、ページ分割されたレポートを Web ページの div 要素に読み込むことができます。

クライアント API を使用する完全なサンプルでは、[プレイグラウンド ツール](https://microsoft.github.io/PowerBI-JavaScript/demo)を使用できます。 プレイグラウンド ツールを使うと、さまざまな種類の Power BI Embedded のサンプルを簡単に試すことができます。 Power BI 埋め込み分析のクライアント API の詳細については、「[Power BI 埋め込み分析のクライアント API](/javascript/api/overview/powerbi/)」ページを参照してください。
