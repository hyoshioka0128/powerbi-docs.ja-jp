...
ページの自動更新は、[DirectQuery ソース](../connect-data/desktop-directquery-about.md)および一部の LiveConnect シナリオで使用できるため、サポートされているデータ ソースに接続している場合にのみ使用できます。 この制限は、ページの自動更新の両方の種類に適用されます。
