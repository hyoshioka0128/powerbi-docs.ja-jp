...
![縦および横向きのモバイル最適化レポートのスクリーンショット。](media/desktop-create-phone-report/desktop-mobile-optimized-report.png)
...
**前提条件**: 最初のステップは、通常の Web ビューでレポートをデザインして作成することです。 レポートを作成したら、スマートフォンとタブレット用にそれを最適化できます。

モバイルに最適化されたビューを作成するには、Power BI Desktop または Power BI サービスでレポートを開きます。 レポートが開いたら、モバイル レイアウト ビューに移動します。
   * Power BI Desktop の **[表示]** リボンを選択し、 **[モバイル レイアウト]** を選択します。
   * Power BI サービスでは、 **[レポートの編集] > [モバイル レイアウト]** を選択します。 [編集] オプションが表示されない場合は、 **[その他のオプション] (...)** を使用します。
...
視覚化は、相互に重ねて配置することができ、ブックマークを使用して対話型のレポートを作成したり、画像の上に視覚化を重ねて魅力的なレポートを作成したりできます。 [[選択項目] ペイン](#set-the-layering-order-of-visuals-on-the-mobile-layout-canvas)で、視覚エフェクトのレイヤーの順序を変更できます。
...
## <a name="set-the-layering-order-of-visuals-on-the-mobile-layout-canvas"></a>モバイル レイアウト キャンバスでの視覚エフェクトのレイヤーの順序を設定する

視覚エフェクトをキャンバスにドラッグするたびに、そのためのレイヤーが、キャンバス上に既に存在する他のすべての視覚エフェクトの上に追加されます。 **[選択項目]** ペインを使用して、レイヤーの順序を変更できます。

**[選択項目]** ペインを開くには、 **[ビュー]** タブの **[ペインを表示する]** セクションにある **[選択]** ボタンをクリックします。 

**[選択項目]** ペインには、キャンバス上のすべての視覚エフェクトの一覧が表示されます。 一覧の順序には、キャンバス上のレイヤーの順序が反映されています。一覧の最初の視覚エフェクトが最上位のレイヤーであり、一覧の最後の視覚エフェクトが最下位のレイヤーです。 順序を変更するには、視覚エフェクトを一覧の別の場所にドラッグ アンド ドロップするか、視覚エフェクトを選択し、矢印ボタンを使用して上下に移動します。

**[選択項目]** ペインの一覧内の各視覚エフェクトには可視性も示されていますが、モバイル レイアウト ビューで可視性を変更することはできません。これは、通常の Web レイアウト ビューで行う必要があります。

![[選択項目] ペインとそれを開く方法を示すスクリーンショット。](media/desktop-create-phone-report/selection-pane-mobile-layout.png)
