...
この強化された使用状況メトリックに関するレポートを表示すると、Power BI によって構築済みのレポートが生成されます。 これには、過去 30 日間のそのコンテンツの使用状況メトリックが含まれています。 このレポートは、おなじみの Power BI レポートに似ています。 アクセス手段 (Web やモバイル アプリなど) に関係なく、エンド ユーザーがアクセス権を受け取った方法に基づいてスライスできます。 レポートが変更されるにつれて、その使用状況メトリックに関するレポートも変わります。 これは毎日新しいデータで更新されます。
...
強化された使用状況メトリックに関するレポートを初めて起動すると、Power BI により、使用状況メトリックに関するレポート データセットが自動的に作成されます。 強化された使用状況メトリックに関するレポートは、そのデータセットに依存します。 その後、このデータセットは Power BI によって毎日更新されます。 更新スケジュールを変更することはできませんが、利用状況の指標データを更新するために Power BI によって使用される資格情報を更新することはできます。 資格情報の有効期限が切れた場合、またはこのデータセットが存在するワークスペースから使用状況メトリックに関するレポートを最初に起動したユーザーを削除した場合に、スケジュールされた更新の再開が必要になることがあります。
...
| **ページ** | **メトリック** | **説明** |
| --- | --- | --- |
| レポートの使用 | レポートの表示/レポートを開く | レポートの表示は、他のユーザーがレポートを開くたびに記録され、レポートに対する固有のランディングを表します。 これにより、"レポートはどのくらいの頻度でアクセスされていますか" という質問への答えが得られます。 このレポートの表示の定義は、以前の使用状況メトリックに関するレポートとは異なります。 レポート ページを変更しても、別のレポートの表示とは見なされなくなりました。 代わりに、レポート ページを変更すると、次のメトリックであるレポート ページの表示がカウントされます。 共有やピン留めなどのアクティビティは、使用状況メトリックでは考慮されなくなりました。 |
| レポートの使用 | レポート ページの表示 | レポート ページの表示は、ユーザーがレポート ページを表示するたびに記録されます。 これは、すべてのページの合計表示回数を表します。 これにより、"レポート ページはどのくらいの頻度でアクセスされていますか" という質問への答えが得られます。 したがって、レポート ページを変更すると、レポート ページの表示がカウントされます。 重要な詳細情報については、「[考慮事項と制限事項](#considerations-and-limitations)」を参照してください。 |
...
| レポートの使用 | 順位 | 順位は、表示回数に基づき、組織内の他のすべてのレポートと比較したレポートの使用頻度を示します。 ランク 1 は、組織内のすべてのレポートの中で、そのレポートの表示回数が最も多いことを意味します。   |
| レポートの使用 | 1 日あたりのレポートの表示回数 | 1 日あたりのレポートの表示の合計数。 レポート レベルでカウントされますが、レポート ページの表示は考慮されません。 |
...
既定では、使用状況メトリックではユーザーごとのデータが有効になっています。 メトリックに関するレポートには、コンテンツ コンシューマー アカウント情報が含まれています。 管理者が一部またはすべてのユーザーに対してこの情報を公開しない場合は、使用状況レポートからユーザー情報を除外できます。 Power BI 管理ポータルのテナント設定では、指定したセキュリティ グループまたは組織全体の使用状況メトリックに対して、ユーザーごとのデータを無効にすることができます。
