...
> [!IMPORTANT]
> SAP では、Power BI のオンプレミス データ ゲートウェイの gx64krb5 がサポートされなくなりました。 その結果、Microsoft は、このシナリオでの gx64krb5 の使用に関するサポートを廃止しました。 既存の接続は引き続き機能しますが、2021 年 2 月以降、このシナリオで新しい接続を作成することはできなくなります。 代わりに CommonCryptoLib を使用してください。 
